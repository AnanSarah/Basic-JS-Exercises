...
function alphabetic_order(word) {
  var sp= word.split("");
  var sorted = sp.sort();
  //return word.split("").sort().join(''); this works as well
  return sp.join('');
}
...
function dash_in_even(number) {
  ...
  for (var i = 0; i < numString.length; i++) {
    dash.push(numString[i]);
   if (Number(numString[i+1]%2) == 0 && (Number(numString[i]%2) ==0))
   {
     dash.push("-");
   }
  }
...
function guessing_game(guess) {
    guess1=Math.floor(Math.random() * 10) + 1;
    if (guess==guess1){
        console.log("matched");
    }
    else{
